...
public class Update5Action extends AbstractAction {
	...
	public String insert(){
		...
		insert_user_profile_table.setDwelling(checkcode(this.dwelling));
		insert_user_profile_table.setName(checkcode(this.name));
		insert_user_character_table.setPersonality((this.personality));
		insert_user_profile_table.setDay(checkcode(this.day));
		insert_user_profile_table.setNewday(checkcode(this.newday));
		insert_user_profile_table.setUserid(checkcode(this.userid));
		insert_user_profile_table.setNewuserid(checkcode(this.newuserid));
		insert_user_profile_table.setDelete((this.delete));
		insert_user_profile_table.setFlg((this.flg));
		insert_user_character_table.setInterest(checkcode(this.interest));
		String[] data = {this.dwelling,this.name,this.day,this.newday,this.userid,this.newuserid,this.interest,this.personality};
		int i=0;
	...
	public String delete() {
		...
		session.beginTransaction();
		try {
			User_Profile profile = (User_Profile)session.load(User_Profile.class, update_id);
			User_Character character = (User_Character) session.load(User_Character.class, update_id);
