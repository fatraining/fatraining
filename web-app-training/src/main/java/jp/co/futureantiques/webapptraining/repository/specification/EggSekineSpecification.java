...
import jp.co.futureantiques.webapptraining.model.form.eggSekine.EggSearchForm;

public class EggSekineSpecification {
	 ...
	 */
	public static Specification<EggMain> generateEggSpecification(final EggSearchForm form) {
		return new Specification<EggMain>() {
			@Override
			public Predicate toPredicate(Root<EggMain> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (form.getCookId() != null && form.getCookId() != 0) {

					// 俳優を条件に追加
					Predicate newCondition = cb.equal(root.get("cookId"), form.getCookId());
					condition = getPredicate(cb, condition, newCondition);
				}
