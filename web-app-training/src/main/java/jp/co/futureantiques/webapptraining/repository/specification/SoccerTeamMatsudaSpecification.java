 ...
 * @author future
 */
public class SoccerTeamMatsudaSpecification {
	...
	public static Specification<TeamMainMatsuda> generateSoccerTeamMatsudaSpecification(
		...
		return new Specification<TeamMainMatsuda>() {
			...
			public Predicate toPredicate(Root<TeamMainMatsuda> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (form.getHometownId() != null && form.getHometownId() != 0) {

					// ホームタウンを条件に追加
					Predicate newCondition = cb.equal(root.get("hometownId"), form.getHometownId());
					condition = getPredicate(cb, condition, newCondition);
				}
				if (form.getTeamColorId() != null && form.getTeamColorId() != 0) {

					// チームカラーを条件に追加
					Predicate newCondition = cb.equal(root.get("teamColorId"), form.getTeamColorId());
					condition = getPredicate(cb, condition, newCondition);
				}
