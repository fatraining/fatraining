...
public class VoiceActorKobaSpecification {
	 ...
	 * @return VoiceMainKobaのSpecification
	 */

	public static Specification<VoiceMainKoba> generateVoiceActorSpecification(final VoiceActorKobaSearchForm form){
		return new Specification<VoiceMainKoba>() {

			@Override
			public Predicate toPredicate(Root<VoiceMainKoba> root,CriteriaQuery<?> query,CriteriaBuilder cb) {
				...
				if(!StringUtils.isEmpty(form.getAnimeName())) {
					// アニメタイトルを条件に追加
					form.setAnimeName(form.getAnimeName().trim());
					Predicate newCondition = cb.like(root.get("animeName"),"%" + form.getAnimeName() + "%");
					condition = getPredicate(cb,condition, newCondition);
				}
