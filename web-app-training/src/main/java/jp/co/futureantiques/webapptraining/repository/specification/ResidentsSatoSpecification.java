...
/**
 * ResidentsMainの検索条件を生成するクラス
 * @author future
 *
 */

public class ResidentsSatoSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param ResidentsSatoSearchForm form
	 * @return ResidentsMainのSpecification
	 */
	public static Specification<ResidentsMainSato> generateResidentsSatoSpecification(final ResidentsSatoSearchForm form) {
		return new Specification<ResidentsMainSato>() {
			@Override
			public Predicate toPredicate(Root<ResidentsMainSato> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(form.getResidentsName())) {

					// 住民名を条件に追加
					form.setResidentsName(form.getResidentsName().trim());
					Predicate newCondition = cb.like(root.get("residentsName"), "%" + form.getResidentsName() + "%");
					condition = getPredicate(cb, condition, newCondition);
				}
