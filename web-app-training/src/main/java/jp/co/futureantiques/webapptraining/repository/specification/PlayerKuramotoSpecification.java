...
/**
 * PlayerMainKuramotoの検索条件を生成するクラス
 *
 * @author future
 */
public class PlayerKuramotoSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param PlayerKuramotoSearchForm form
	 * @return PlayerKuramotoのSpecification
	 */
	public static Specification<PlayerMainKuramoto> generatePlayerSpecification(final PlayerKuramotoSearchForm form) {
		return new Specification<PlayerMainKuramoto>() {
			@Override
			public Predicate toPredicate(Root<PlayerMainKuramoto> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (form.getBtId() != null && form.getBtId() != 0) {

					// 投打を条件に追加
					Predicate newCondition = cb.equal(root.get("btId"), form.getBtId());
					condition = getPredicate(cb, condition, newCondition);
				}
