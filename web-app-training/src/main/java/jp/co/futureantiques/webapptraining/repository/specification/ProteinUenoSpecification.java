...
public class ProteinUenoSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param ProteinUenoSearchForm form
	 * @return ProteinMainのSpecification
	 */
	public static Specification<ProteinMainUeno> generateProteinSpecification(final ProteinUenoSearchForm form) {
		return new Specification<ProteinMainUeno>() {

			@Override
			public Predicate toPredicate(Root<ProteinMainUeno> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(form.getFlavor())) {

					// フレーバーを条件に追加
					form.setFlavor(form.getFlavor().trim());
					Predicate newCondition = cb.like(root.get("flavor"), "%" + form.getFlavor() + "%");
					condition = getPredicate(cb, condition, newCondition);
				}
				if (form.getKindId() != null && form.getKindId() != 0) {
