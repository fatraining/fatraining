...
/**
 * RuinsMainKuriharaの検索条件を生成するクラス
 *
 * @author M.Kurihara
 */
public class RuinsKuriharaSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param RuinsKuriharaSearchForm form
	 * @return RuinsMainKuriharaのSpecification
	 */
	public static Specification<RuinsMainKurihara> generateRuinsSpecification(final RuinsKuriharaSearchForm form) {
		return new Specification<RuinsMainKurihara>() {
			@Override
			public Predicate toPredicate(Root<RuinsMainKurihara> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(form.getRuinsName())) {

					// 廃墟名を条件に追加
					form.setRuinsName(form.getRuinsName().trim());
					Predicate newCondition = cb.like(root.get("ruinsName"), "%" + form.getRuinsName() + "%");
					condition = getPredicate(cb, condition, newCondition);
				}
