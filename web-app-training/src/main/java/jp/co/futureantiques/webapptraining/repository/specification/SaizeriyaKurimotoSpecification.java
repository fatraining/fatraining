...
public class SaizeriyaKurimotoSpecification {
	...
	public static Specification<SaizeriyaMenuMainKurimoto> generateSaizeriyaSpecification(final SaizeriyaKurimotoSearchForm form) {
		return new Specification<SaizeriyaMenuMainKurimoto>() {
			@Override
			public Predicate toPredicate(Root<SaizeriyaMenuMainKurimoto> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(form.getMenuName())) {

					// メニュー名を条件に追加
					form.setMenuName(form.getMenuName().trim());
					Predicate newCondition = cb.like(root.get("menuName"), "%" + form.getMenuName() + "%");
					condition = getPredicate(cb, condition, newCondition);
				}
				...
				if (form.getPriceId() != null && form.getPriceId() != 0) {

					// 価格帯を条件に追加
					Predicate newCondition = cb.equal(root.get("priceId"), form.getPriceId());
					condition = getPredicate(cb, condition, newCondition);
				}
