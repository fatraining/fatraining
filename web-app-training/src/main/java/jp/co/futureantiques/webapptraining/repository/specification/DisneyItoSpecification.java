...
/**
 * DisneyMainItoの検索条件を生成するクラス
 *
 * @author Ayaka Ito
 */

public class DisneyItoSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param DisneyItoSearchForm form
	 * @return DisneyMainItoのSpecification
	 */
	public static Specification<DisneyMainIto> generateDisneyItoSpecification(final DisneyItoSearchForm form) {
		return new Specification<DisneyMainIto>() {
			@Override
			public Predicate toPredicate(Root<DisneyMainIto> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (form.getParkId() != null && form.getParkId() != 0) {

					// パークを条件に追加
					Predicate newCondition = cb.equal(root.get("parkId"), form.getParkId());
					condition = getPredicate(cb, condition, newCondition);
				}
