...
import lombok.Data;
	...
	/*	public mcuMainHayashi convertToMcuMainHayashiForUpdate(mcuMainHayashi mcuMainHayashi) {
		 ...
		 */
		/*private McuMainHayashi convertToMcuMainHayashi(McuMaunHayashi McuMaunHayashi) {
			McuMaunHayashi.setCharacterName(this.characterName);
			McuMaunHayashi.setTypeId(this.typeId);
			McuMaunHayashi.setSexId(this.sexId);
			McuMaunHayashi.setAppearanceYear(Integer.parseInt(this.appearanceYearStr));
			if (!this.appearanceTimeStr.isEmpty()) {

				// 登場回数が入力されていた場合
				mcuMaunHayashi.setAppearanceTime(Integer.parseInt(this.appearanceTimeStr));
			} else {
		...
		return mcuMainHayashiSpecification;
		}
