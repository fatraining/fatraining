...
/**
 * BoowyMainWakiの検索条件を生成するクラス
 *
 * @author WAKI
 */
public class BoowyWakiSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param BoowyWakiSearchForm form
	 * @return BoowyMainWakiのSpecification
	 */
	public static Specification<BoowyMainWaki> generateBoowyWakiSpecification(final BoowyWakiSearchForm form) {
		return new Specification<BoowyMainWaki>() {
			@Override
			public Predicate toPredicate(Root<BoowyMainWaki> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (form.getSongwriterId() != null && form.getSongwriterId() != 0) {

					// 作詞者を条件に追加
					Predicate newCondition = cb.equal(root.get("songwriterId"), form.getSongwriterId());
					condition = getPredicate(cb, condition, newCondition);
				}
