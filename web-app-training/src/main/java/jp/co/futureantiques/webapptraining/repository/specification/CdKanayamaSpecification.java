...
public class CdKanayamaSpecification {

	/**
	 * 検索条件生成の実装
	 *
	 * @param CdKanayamaSearchForm form
	 * @return CdMainのSpecification
	 */
	public static Specification<CdMainKanayama> generateCdSpecification(final CdKanayamaSearchForm form) {
		return new Specification<CdMainKanayama>() {

			@Override
			public Predicate toPredicate(Root<CdMainKanayama> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if (!StringUtils.isEmpty(form.getCdTitle())) {

					// CDタイトルを条件に追加
					form.setCdTitle(form.getCdTitle().trim());
					Predicate newCondition = cb.like(root.get("cdTitle"), "%" + form.getCdTitle() + "%");
					condition = getPredicate(cb, condition, newCondition);
				}
